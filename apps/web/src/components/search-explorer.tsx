    ...
    <div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            {hasFilters && (
              <button
                onClick={clearAll}
                className="inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-xs font-medium text-slate2 transition hover:bg-line/60 hover:text-navy"
              >
                <X className="h-3.5 w-3.5" /> {t('clear')}
              </button>
            )}
            <label className="relative inline-flex items-center">
              <ArrowUpDown className="pointer-events-none absolute left-3 h-3.5 w-3.5 text-slate2" />
              <select
                value={sort}
              ...
              <svg className="pointer-events-none absolute right-3 h-3 w-3 text-slate2" viewBox="0 0 12 12" fill="none">
      ...
      {results.length === 0 ? (
        <div className="rounded-2xl border border-line bg-surface p-10 text-center">
          <p className="text-slate2">{t('empty')}</p>
          {hasFilters && (
            <button
              onClick={clearAll}
    ...
    >
      ...
      <span
        className={`rounded-full px-1.5 text-[11px] font-semibold tabular-nums ${
          active ? 'bg-white/25 text-white' : 'bg-line/70 text-slate2'
        }`}
      >
        {count}
      </span>
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-sm font-medium transition ${
        active ? 'bg-brand text-white shadow-sm' : 'text-slate2'
      }`}
    >
      {children}
    </button>
...
function ResultRow({
  ...
  return (
    <div
      ...
      className={`group flex cursor-pointer gap-3.5 rounded-2xl border bg-white p-3 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-card ${
    ...
    >
      <div className="relative h-[104px] w-[112px] shrink-0 overflow-hidden rounded-xl bg-bg">
        ...
        <span className="chip absolute bottom-1.5 left-1.5 bg-navy/75 backdrop-blur text-white !px-2 !py-0.5">
      ...
      <div className="min-w-0 flex-1 py-0.5">
        ...
        {v.category && (
          <p className="mt-0.5 flex items-center gap-1 text-xs text-slate2">
